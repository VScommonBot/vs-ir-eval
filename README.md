...
벤처스퀘어의 초기 스타트업 투자 및 멘토링 철학을 바탕으로 사업계획서(IR)를 평가하는 전문 프레임워크 스킬입니다.
...
2026-05 업데이트로 Sequoia, Y Combinator, a16z, Bessemer의 공개 투자/피치/지표 프레임워크를 보조 렌즈로 접목했습니다. 단, 최종 판단 기준은 벤처스퀘어 투자 철학을 우선합니다.
...
10. **💡 벤처스퀘어 관점의 멘토링**: 창업자에게 던질 압박 질문 3가지, 넥스트 스텝 조언
11. **📄 산출물/공유/DB 처리 계획**: Word 보고서, Drive 업로드, Google Chat 공유, DB 확장, 경쟁사 백그라운드 수집 큐
12. **🗂️ 미팅/IR 질문 박스와 업데이트 루프**: 맞춤 질문, 답변 기록, 녹취/회의록 연결, 보고서 버전 업데이트, 모의 투자심사 가이드
...
벤처스퀘어 투자심사는 1회성 보고서가 아니라, 질문과 답변을 통해 기업 이해도를 계속 높이는 과정입니다.

- 각 기업에 맞는 조언과 질문을 보고서 하단 박스 형태로 남긴다.
- 질문은 `질문 의도`, `좋은 답변 기준`, `답변 기록란`, `후속 확인자료`와 함께 구성한다.
- 실제 미팅이나 IR 수행 시 질문별 답변을 기록하고, 녹취/회의록/요약 메모를 연결한다.
- 답변은 `확인 필요`, `검증됨`, `리스크 확대` 등으로 상태화한다.
- 답변과 증빙을 바탕으로 점수, 투자의견, 리스크, 밸류에이션, TIPS/LIPS 적합도를 업데이트한다.
- 보고서는 `v1_서면검토`, `v2_IR미팅후`, `v3_증빙확인후`처럼 버전 관리한다.
- 축적된 질문/답변을 바탕으로 스타트업에게 모의 투자심사 가이드를 제공한다.
- DB 확장 엔티티로 `investment_questions`, `meeting_notes`, `meeting_recordings`, `question_answers`, `report_revisions`, `mock_review_sessions`를 권장한다.
