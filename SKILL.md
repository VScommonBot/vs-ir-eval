...
AI 간이 투자심사보고서는 채팅 답변만으로 끝내지 않고, 업무 산출물과 데이터 확장까지 이어지는 흐름을 기본값으로 둡니다.

1. **MS Word 보고서 생성**
   - 최종 보고서는 `.docx` 형식의 MS Word 문서로 작성한다.
   - 벤처스퀘어 디자인 시스템 가이드를 반영한다. 문서형 산출물에서는 과한 장식보다 명확한 제목 체계, 표, 근거 링크, 투자판단 요약, 리스크 박스를 우선한다.
   - 권장 구성: 표지, Executive Summary, 점수표, 가점/감점, 시장/경쟁사, 밸류에이션, TIPS/LIPS, 체크리스트, 출처/부록.
2. **Google Drive 업로드**
   - 생성한 Word 문서는 `mse@venturesquare.net` Drive에 업로드한다.
   - 파일명은 `YYYYMMDD_기업명_AI간이투자심사보고서_vN.docx` 형식을 권장한다.
   - 외부 공유 범위는 기본 비공개 또는 도메인 정책을 따르며, 공개/외부 공유는 사용자 승인 후 진행한다.
3. **Google Chat 공유**
   - Google Chat 스페이스에는 문서 링크와 핵심 요약만 공유한다.
   - 공유 요약은 5줄 이내를 기본으로 한다. 예: 투자의견, 핵심 근거, 주요 리스크, 밸류에이션 범위, 다음 액션.
   - 민감한 원문, 재무 세부, 개인정보, 비공개 계약 내용은 Chat 본문에 길게 노출하지 않는다.
4. **관계형 DB 확장**
   - 보고서 문서 링크와 메타데이터를 DB에 저장할 수 있도록 구조화한다.
   - 권장 엔티티: `companies`, `people`, `investment_reports`, `report_documents`, `competitors`, `company_competitors`, `funding_events`, `ma_events`, `ipo_comparables`, `valuation_estimates`, `sources`.
   - DB write는 별도 승인 또는 기존 파이프라인 정책이 있을 때만 수행한다. 기본은 JSON/SQL preview를 만들고 검토 후 반영한다.
5. **경쟁사 백그라운드 수집**
   - 경쟁사 분석에 등장한 기업은 백그라운드 수집 대상으로 표시한다.
   - 각 경쟁사에 대해 회사명, 국가, 웹사이트, 사업영역, 제품, 고객군, 투자/상장/M&A 이벤트, 출처 URL, 신뢰도를 구조화한다.
   - 수집된 경쟁사 정보는 추후 투자심사, 기사, DB, 시장지도에 재사용 가능하도록 관계형 데이터로 확장한다.

## Organic Review Loop

벤처스퀘어 투자심사는 1회성 보고서가 아니라, 질문·미팅·답변·녹취·업데이트가 반복되는 유기적 과정으로 설계합니다.

1. **기업별 조언/질문 박스**
   - 각 기업에 맞는 조언과 질문을 보고서 하단 박스 형태로 남긴다.
   - 질문은 일반론이 아니라 해당 기업의 핵심 리스크, 가설, 고객, BM, 규제, 경쟁, 밸류에이션을 검증하도록 작성한다.
   - 각 질문에는 `질문 의도`, `좋은 답변의 기준`, `답변 기록란`, `후속 확인자료`를 붙인다.
2. **미팅/IR 답변 기록**
   - 실제 미팅이나 IR 수행 시 질문별 답변을 기록한다.
   - 가능하면 녹취/회의록/요약 메모를 연결하고, 발언자·날짜·맥락을 남긴다.
   - 답변이 모호하면 `확인 필요`, 증빙이 있으면 `검증됨`, 상충되면 `리스크 확대`로 표시한다.
3. **보고서 업데이트**
   - 미팅 답변과 녹취를 바탕으로 점수, 투자의견, 리스크, 밸류에이션, TIPS/LIPS 적합도를 업데이트한다.
   - 변경 전후 판단이 달라진 이유를 `업데이트 로그`로 남긴다.
   - 기존 내용을 덮어쓰기보다 버전 관리한다. 예: `v1_서면검토`, `v2_IR미팅후`, `v3_증빙확인후`.
4. **모의 투자심사 구성**
   - 축적된 질문/답변을 바탕으로 스타트업에게 모의 투자심사 과정을 제공한다.
   - 구성: 예상 질문, 좋은 답변 기준, 약한 답변 예시, 보완자료 목록, 다음 IR 전 숙제.
   - 목적은 단순 평가가 아니라 투자 유치 준비도를 높이는 가이드 제공이다.
5. **DB 연계**
   - 질문, 답변, 녹취 링크, 보고서 버전, 업데이트 로그를 관계형 데이터로 연결한다.
   - 권장 엔티티: `investment_questions`, `meeting_notes`, `meeting_recordings`, `question_answers`, `report_revisions`, `mock_review_sessions`.
...
## 11. 📄 산출물/공유/DB 처리 계획
- **Word 보고서**: (파일명, 주요 섹션, 디자인 시스템 적용 방식)
- **Drive 업로드**: (업로드 계정/폴더, 공유 범위, 링크)
- **Google Chat 공유 요약**: (5줄 이내 요약)
- **DB 확장 대상**: (보고서, 기업, 인물, 경쟁사, 투자/M&A/IPO, 출처)
- **백그라운드 수집 큐**: (경쟁사/비교기업 목록과 수집 우선순위)

## 12. 🗂️ 미팅/IR 질문 박스와 업데이트 루프
- **기업별 조언 메모**: (하단 박스에 넣을 맞춤 조언)
- **핵심 질문 박스**: (질문, 질문 의도, 좋은 답변 기준, 답변 기록란, 후속 확인자료)
- **미팅/녹취 기록 계획**: (IR/미팅 일자, 참석자, 녹취/회의록 링크, 기록 방식)
- **답변 반영 업데이트**: (점수/리스크/밸류에이션/TIPS 판단 업데이트 방식)
- **모의 투자심사 가이드**: (예상 질문, 약한 답변 보완법, 다음 IR 전 숙제)
...
8. 사용자가 보고서 산출을 요청했거나 기본 운영 플로우가 허용되면 Word 보고서를 만들고 Drive 업로드, Google Chat 공유 요약, DB preview, 경쟁사 수집 큐를 준비합니다.
9. 기업별 질문 박스와 답변 기록란을 만들고, 미팅/IR 이후 녹취·메모를 반영해 보고서 버전을 업데이트할 수 있게 구성합니다.
10. Google Chat 발송, 외부 공유, DB write처럼 외부 시스템에 남는 작업은 승인/권한/정책을 확인한 뒤 수행합니다.
