...
2026-06 업데이트에서는 David Kim의 *Nobody Understands by Watching: The Radical Empathy Method for Problems You Cannot See* (Field Edition, OPENSEE, 2026)에서 제시한 Do Gap / Radical Empathy 렌즈를 추가했습니다. 이 렌즈는 공개 멘토링과 투자심사 준비에서 창업팀이 고객을 들은 수준인지, 실제 현장에서 보고 고객 업무를 살아본 수준인지를 구분하는 증거 품질 기준입니다.

## 공개용과 내부용 기능 구분

새 평가 항목, 워크플로우, 출력 필드, 자동화를 추가할 때는 먼저 범위를 분류합니다.

> 이 기능은 공개용 스킬, 벤처스퀘어 내부용 스킬, 또는 양쪽 모두 중 어디에 포함할까요?

공개용 기능은 창업자 코칭, 공개 데모, 외부 공유 가능한 피드백, 근거 기반 준비 가이드에 한정합니다. 내부용 기능은 벤처스퀘어 DB 조회, 관계 이력, 투자심의 워크플로우, Gmail/Drive/Google Chat 처리, 보고서 업로드·전달, 내부 coverage gate, revision logging을 포함합니다.

이 공개 저장소는 DB write, Drive 업로드, 이메일 회신, Google Chat/Telegram 발송, n8n 워크플로우 변경, 내부 보고서 전달 같은 사이드 이펙트를 직접 수행하지 않습니다. 해당 작업은 비공개 내부 스킬 또는 명시적으로 승인된 내부 자동화에 둡니다.
...
- 숫자 점수는 근거를 정렬하는 보조값입니다. 공개 판단 단위는 5단계 등급과 근거 추적입니다.
...
투자심사와 멘토링에서 고객 발언과 고객 이해를 분리하기 위해 사용합니다.
...
> 팀이 고객의 일을 직접 보거나 해봤기 때문에 알게 된 것 중, 보고서·설문·IR 인터뷰만으로는 절대 알 수 없었던 사실은 무엇인가?

이 렌즈는 별도 투자 결정 기준이 아닙니다. AI, 자동화, 헬스케어, 물류, 금융, 교육, 공공 DX, 해외진출/로컬라이제이션 사례에서 증거 품질을 확인하기 위한 공개 멘토링 기준입니다.
...
공개 고객 수용성 가설을 보조하기 위해 [Nemotron-Personas-Korea](https://huggingface.co/datasets/nvidia/Nemotron-Personas-Korea)를 선택적으로 사용합니다. `hf datasets sql` 또는 DuckDB로 필요한 세그먼트만 원격 필터링하고 제한된 표본을 고정 프롬프트로 평가합니다. 데이터셋 URL, revision, 세그먼트 정의, 표본 수, 프롬프트 버전, 실행일을 기록합니다.

고객 수용성 방향, 주요 반론, 가격 저항, 대체 행동, `합성 페르소나의 가상 응답`, 실제 인터뷰·PoC 검증질문을 별도 보고합니다. 합성 응답을 실제 고객 인용·시장 예측·투자 근거로 표현하지 않습니다. 초기에는 클라우드 원격 조회를 사용하고 반복량이 높아질 때만 revision 고정 로컬 snapshot으로 전환합니다. CC BY 4.0 및 NVIDIA 출처를 표시합니다.
...
업력별 핵심 증빙:
- **예비창업**: 창업자-문제 적합성, 고객 인터뷰, 고객 관찰 기록, 고객 업무 직접 수행 경험, MVP 계획, 초기 자금 계획
- **3년 미만**: MVP/PoC, 첫 유상 고객, 사용 로그, 현장 업무 흐름 검증, 기술/규제/IP 검증
...
- 강한 Do Gap 증거가 있음. 즉, 팀이 고객 업무를 관찰하거나 직접 살아봤고 보고서·설문·IR 인터뷰로는 알 수 없던 인사이트를 말할 수 있음
- 배포 부담이 낮음. 즉, 기존 고객 업무 흐름에 들어가면서 숨은 감시, 연동, 예외처리, 교육, 지원 부담을 크게 늘리지 않음
...
- 설문, 인터뷰, LOI, 말로 표현된 관심은 있지만 실제 관찰 행동, 현장 PoC, 사용 로그, 반복 사용, 유상 사용 증거가 부족함
- 데모는 작동하지만 실제 도입 시 고객에게 추가 단계, 화면, 승인, 교육, 감시, 수동 보정 업무를 만듦
